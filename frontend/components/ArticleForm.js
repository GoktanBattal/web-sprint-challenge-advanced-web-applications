...
export default function ArticleForm(props) {
  ...
  const isDisabled = () => {
    ...
    let titleCondition = values.title.trim().length >= 1;
    let textCondition = values.text.trim().length >= 1;
    let topicCondition = values.topic === 'React' || values.topic === 'JavaScript' || values.topic === 'Node';

    return !(titleCondition && textCondition && topicCondition);
  };
